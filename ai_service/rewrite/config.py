...
"""
Quản lý cấu hình cho rewrite service từ file .env
...
REWRITE_DIR = Path(__file__).resolve().parent  # Đường dẫn đến thư mục rewrite

# Đường dẫn đến các file .env
ROOT_ENV_FILE = ROOT_DIR / ".env"
AI_SERVICE_ENV_FILE = AI_SERVICE_DIR / ".env"
REWRITE_ENV_FILE = REWRITE_DIR / ".env"
...
def load_config():
    ...
    1. .env trong thư mục rewrite (nếu có)
    ...
    # Tải file .env từ thư mục rewrite nếu có (ưu tiên cao nhất)
    if REWRITE_ENV_FILE.exists():
        logger.info(f"Tải cấu hình từ file {REWRITE_ENV_FILE}")
        load_dotenv(dotenv_path=REWRITE_ENV_FILE, override=True)
